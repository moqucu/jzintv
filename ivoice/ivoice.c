...
/* ======================================================================== */
LOCAL INLINE int16_t  limit (int16_t s);
LOCAL INLINE uint32_t bitrev(uint32_t val);
LOCAL int             lpc12_update(lpc12_t *f, int, int16_t *, uint32_t *);
LOCAL void            lpc12_regdec(lpc12_t *f);
LOCAL uint32_t        sp0256_getb(ivoice_t *ivoice, int len);
LOCAL void            sp0256_micro(ivoice_t *iv);
LOCAL void            iv_smp_start (ivoice_t *iv);
LOCAL void            iv_smp_stop  (ivoice_t *iv);
LOCAL INLINE void     iv_smp_record(ivoice_t *iv, int ns, int16_t *out, int id);

LOCAL const uint8_t wav_hdr[44] =
{
    0x52, 0x49, 0x46, 0x46, /* "RIFF"                                       */
    0x00, 0x00, 0x00, 0x00, /* Total file length - 8                        */
    0x57, 0x41, 0x56, 0x45, /* "WAVE"                                       */
    0x66, 0x6D, 0x74, 0x20, /* "fmt "                                       */
    0x10, 0x00, 0x00, 0x00, /* 0x10 == PCM                                  */
    0x01, 0x00,             /* 0x01 == PCM                                  */
    0x01, 0x00,             /* 1 channel                                    */
    0x10, 0x27, 0x00, 0x00, /* Sample rate == 10kHz                         */
    0x20, 0x4E, 0x00, 0x00, /* Byte rate == 20 kB/sec                       */
    0x02, 0x00,             /* Block alignment:  channels * bits/sample / 8 */
    0x10, 0x00,             /* Bits/sample = 16.                            */
    0x64, 0x61, 0x74, 0x61, /* "data"                                       */
    0x00, 0x00, 0x00, 0x00  /* Total sample data length                     */
};

/* ======================================================================== */
/*  IV_SMP_UPHDR -- Update length field in header for each sample block.    */
/* ======================================================================== */
LOCAL void iv_smp_uphdr(ivoice_t *iv)
{
    ...
    /* -------------------------------------------------------------------- */
    memcpy(upd_wav_hdr, wav_hdr, 44);

    tot_smp_bytes  = iv->smp_totsmp * 2;
    tot_file_bytes = tot_smp_bytes + 36;
    ...
    /* -------------------------------------------------------------------- */
    if (fseek(iv->smp_file, 0, SEEK_SET) == 0)
    {
        fwrite(upd_wav_hdr, 44, 1, iv->smp_file);
        fseek(iv->smp_file,  0, SEEK_END);
    }
}

/* ======================================================================== */
/*  IV_SMP_START -- Start a new sample recording file.                      */
/* ======================================================================== */
LOCAL void iv_smp_start(ivoice_t *iv)
{
    int tries;
    ...
    /* -------------------------------------------------------------------- */
    /*  We don't have a filename, don't do anything.                        */
    /* -------------------------------------------------------------------- */
    if (!iv->smp_tname)
        return;

    /* -------------------------------------------------------------------- */
    /*  Stop recording previous sample if two samples back-to-back.         */
    /* -------------------------------------------------------------------- */
    if (iv->smp_file)
        iv_smp_stop(iv);

    /* -------------------------------------------------------------------- */
    /*  Generate a filename for the current sequence number.                */
    /* -------------------------------------------------------------------- */
    tries = 10;
    iv->smp_number--;
    do
    {
        iv->smp_number++;
        iv->smp_cksum = (uint32_t *)
                            realloc(iv->smp_cksum,
                                    sizeof(uint32_t)*(iv->smp_number+1));
        iv->smp_cksum[iv->smp_number] = 0x00000000;

        sprintf(iv->smp_cname, "%s%.4d.wav", iv->smp_tname, iv->smp_number);
        if (file_exists(iv->smp_cname))
            continue;

        iv->smp_file = fopen(iv->smp_cname, "wb");
    } while (!iv->smp_file && tries-->0);

    iv->smp_cursum = 0;
    iv->smp_totsmp = 0;
    if (iv->smp_file)
    {
        /* Write an initial WAV header w/out size information initially. */
        fwrite(wav_hdr, 1, sizeof(wav_hdr), iv->smp_file);
        jzp_clear_and_eol(
                jzp_printf("ivoice:  starting sample: %s", iv->smp_cname));
        jzp_flush();
    }
}

/* ======================================================================== */
/*  IV_SMP_STOP  -- Stop a sample recording file.                           */
/* ======================================================================== */
LOCAL void iv_smp_stop(ivoice_t *iv)
{
    int i;
    ...
    /* -------------------------------------------------------------------- */
    /*  We don't have a filename or file, don't do anything.                */
    /* -------------------------------------------------------------------- */
    if (!iv->smp_tname || !iv->smp_file)
        return;

    /* -------------------------------------------------------------------- */
    /*  Write the file length to the header and close the file.             */
    /* -------------------------------------------------------------------- */
    iv_smp_uphdr(iv);
    fclose(iv->smp_file);
    iv->smp_file = NULL;
    ...
    jzp_clear_and_eol(
        jzp_printf("ivoice:  stopping sample: %s  CRC: %.8X  len: %d",
                   iv->smp_cname, iv->smp_cursum, iv->smp_totsmp));

    /* -------------------------------------------------------------------- */
    /*  If this file was unique, keep it.  Otherwise unlink it.             */
    /* -------------------------------------------------------------------- */
    i = 0;
    if (iv->smp_cksum)
        for (i = 0; i < iv->smp_number; i++)
            if (iv->smp_cksum[i] == iv->smp_cursum)
                break;

    /* -------------------------------------------------------------------- */
    /*  If we found a match, so don't keep it.                              */
    /* -------------------------------------------------------------------- */
    if (i != iv->smp_number)
    {
        jzp_printf("ivoice:  sample was a repeat of earlier sample: %d\n", i);
        unlink(iv->smp_cname);
        return;
    }


    iv->smp_cksum = (uint32_t*)realloc(iv->smp_cksum,
                                       sizeof(uint32_t)*(iv->smp_number+1));
    iv->smp_cksum[iv->smp_number++] = iv->smp_cursum;
}

/* ======================================================================== */
/*  IV_SMP_RECORD -- Add a sample to a current sample record file.          */
/* ======================================================================== */
LOCAL INLINE void iv_smp_record(ivoice_t *iv, int num_samp, int16_t *out,
                                int oidx)
{
    uint8_t buf[SCBUF_SIZE * 2];
    ...
    if (iv->smp_file)
    {
        ...
        {
            buf[2*i + 0] =  out[(i+oidx) & SCBUF_MASK]       & 0xFF;
            buf[2*i + 1] = (out[(i+oidx) & SCBUF_MASK] >> 8) & 0xFF;
        }
        fwrite(buf, 2, num_samp, iv->smp_file);
        iv->smp_totsmp += num_samp;
        iv_smp_uphdr(iv);  /* fix bug where early exit leaves 0 len in hdr. */
    }
}

LOCAL INLINE void iv_smp_ckupd(ivoice_t *iv, uint8_t byte)
{
    iv->smp_cursum = CRC32_UPDATE(iv->smp_cursum, byte);
}
...
/* ======================================================================== */
LOCAL int samp_mpy(int coef, int samp)
{
    return coef * samp;
}
#endif
...
/* ======================================================================== */
LOCAL int amp_decode(uint8_t a)
{
    /* -------------------------------------------------------------------- */
...
/* ======================================================================== */
LOCAL int lpc12_update(lpc12_t *f, int num_samp, int16_t *out, uint32_t *optr)
{
    int i, j;
    int16_t samp;
    int do_int, bit;
...
/*LOCAL int stage_map[6] = { 4, 2, 0, 5, 3, 1 };*/
/*LOCAL int stage_map[6] = { 3, 0, 4, 1, 5, 2 };*/
/*LOCAL int stage_map[6] = { 3, 0, 1, 4, 2, 5 };*/
LOCAL const int stage_map[6] = { 0, 1, 2, 3, 4, 5 };
...
LOCAL const uint16_t sp0256_datafmt[] =
{
    /* -------------------------------------------------------------------- */
    /*  OPCODE 1111: PAUSE                                                  */
    /* -------------------------------------------------------------------- */
...
LOCAL const int16_t sp0256_df_idx[16 * 8] =
{
...
/* ======================================================================== */
/*  IVOICE_RDROM -- Tries to read a ROM file in the current directory.      */
/* ======================================================================== */
LOCAL int ivoice_rdrom(ivoice_t *iv, int page)
{
    uint8_t *rom;
    char buf[32];
    LZFILE *f;
    ...
    /* -------------------------------------------------------------------- */
    snprintf(buf, sizeof(buf), "sp0256_%.1X.bin", page);
    ...
    jzp_printf("ivoice:  added %s at SP0256 address $%.4X.0\n", buf, page<<12);
...
/* ======================================================================== */
LOCAL uint32_t ivoice_tk(periph_t *per, uint32_t len)
{
    ivoice_t *ivoice = PERIPH_AS(ivoice_t, per);
    uint64_t until = (per->now + len) * 4;
    ...
    if (!ivoice->cur_buf)
    {
        if (ivoice->snd_buf.num_clean)
        {
            int clean_idx = --ivoice->snd_buf.num_clean;
            ivoice->cur_buf = ivoice->snd_buf.clean[clean_idx];
            ivoice->snd_buf.clean[clean_idx] = NULL;
            ivoice->cur_len = 0;
        } else
            return 0;   /* No buffer available, so time doesn't advance.    */
    }
    ...
    while (ivoice->sound_current < until)
    {
        ...
        while (ivoice->sc_tail < ivoice->sc_head)
        {
            ...
            while (ivoice->sample_frc > sys_clock)
            {
                ...
                /* -------------------------------------------------------- */
                if (ivoice->cur_len >= ivoice->snd_buf.snd->buf_size)
                {
                    /* ---------------------------------------------------- */
                    /*  It's full.  Put it on the dirty list.               */
                    /* ---------------------------------------------------- */
                    ivoice->snd_buf.dirty[ivoice->snd_buf.num_dirty++] =
                        ivoice->cur_buf;

                    /* ---------------------------------------------------- */
                    /*  Try to get a clean buffer.                          */
                    /* ---------------------------------------------------- */
                    if (ivoice->snd_buf.num_clean == 0)
                    {
                        /* ------------------------------------------------ */
                    ...
                    /* ---------------------------------------------------- */
                    ivoice->cur_buf =
                        ivoice->snd_buf.clean[--ivoice->snd_buf.num_clean];
                    ivoice->cur_len = 0;
                }
            }
        }
        ...
        if (samples > 0) do
        {
            ...
            } else
            {
                did_samp += lpc12_update(&ivoice->filt, do_samp,
                 ...
                 samples > did_samp);
        if (did_samp)
            iv_smp_record(ivoice, did_samp, ivoice->scratch, old_idx);
...
//      ivoice->snd_buf.drop++;
    ...
    return (ivoice->sound_current >> 2) - per->now;
}
...
/* ======================================================================== */
LOCAL uint32_t ivoice_rd(periph_t *per, periph_t *ign,
...
/* ======================================================================== */
LOCAL void ivoice_wr(periph_t *const per, periph_t *ign,
                     uint32_t addr, uint32_t data)
{
    ivoice_t *const ivoice = PERIPH_AS(ivoice_t, per);
    UNUSED(ign);
...
/* ======================================================================== */
LOCAL void ivoice_reset(periph_t *const p)
{
    /* -------------------------------------------------------------------- */
    /*  Do a software-style reset of the Intellivoice.                      */
    /* -------------------------------------------------------------------- */
    ivoice_wr(p, NULL, 1, 0x400);
}

/* ======================================================================== */
/*  IVOICE_DTOR  -- Destroy an Intellivoice                                 */
/* ======================================================================== */
LOCAL void ivoice_dtor(periph_t *const p)
{
    ivoice_t *const ivoice = PERIPH_AS(ivoice_t, p);

    CONDFREE(ivoice->window);
    CONDFREE(ivoice->scratch);
    CONDFREE(ivoice->smp_tname);
    CONDFREE(ivoice->smp_cname);
    CONDFREE(ivoice->smp_cksum);
}
...
(
    ...
    const char     *smp_tname,  /*  Filename template for saving samples.   */
...
)
{
    ...
    /* -------------------------------------------------------------------- */
    ivoice->periph.read      = ivoice_rd;
    ivoice->periph.write     = ivoice_wr;
    ivoice->periph.peek      = ivoice_rd;
    ivoice->periph.poke      = ivoice_wr;
    ivoice->periph.tick      = ivoice_tk;
    ivoice->periph.reset     = ivoice_reset;
    ivoice->periph.min_tick  = 1024 * time_scale;
    ivoice->periph.max_tick  = 14934 * time_scale;
    ivoice->periph.addr_base = addr;
    ivoice->periph.addr_mask = 0xFFFF;
    ...
    ivoice->filt.rng   = 1;
    ivoice->wind       = wind;
    ivoice->window     = CALLOC(int, wind);
    ivoice->wind_sum   = 0;
    ivoice->pal_mode   = pal_mode;
    ivoice->time_scale = time_scale;
    if (!ivoice->window)
    {
        fprintf(stderr, "ivoice:  Out of memory allocating sliding window.\n");
        return -1;
    }
    ...
    /* -------------------------------------------------------------------- */
    if (snd_register(AS_PERIPH(snd), &ivoice->snd_buf))
        return -1;

    /* -------------------------------------------------------------------- */
    /*  Set up our initial working buffer.                                  */
    /* -------------------------------------------------------------------- */
    ivoice->cur_buf = ivoice->snd_buf.clean[--ivoice->snd_buf.num_clean];
    ivoice->cur_len = 0;

    /* -------------------------------------------------------------------- */
    /*  Allocate a scratch buffer for generating 10kHz samples.             */
    /* -------------------------------------------------------------------- */
    ivoice->scratch = CALLOC(int16_t, SCBUF_SIZE);
    ...
    /* -------------------------------------------------------------------- */
    for (i = 0; i < 16; i++)
        if (ivoice_rdrom(ivoice, i))
            return -1;

    /* -------------------------------------------------------------------- */
    /*  If the user requested us to save samples, then set things up so     */
    /*  that that can happen.                                               */
    /* -------------------------------------------------------------------- */
    if (smp_tname)
    {
        if ((ivoice->smp_tname = strdup(smp_tname)) != NULL)
        {
            if ((ivoice->smp_cname = CALLOC(char, strlen(smp_tname)+10))==NULL)
            {
                free(ivoice->smp_tname);
                ivoice->smp_tname = NULL;
            }
        }
    }
