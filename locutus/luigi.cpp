                              ...
                              t_byte_vec::const_iterator  end  )
{
    ...
    {
        char buf[10];
        snprintf(buf, 10, "%.2X vs. %.2X", encoded[3], crc_header.get());
    ...
    {
        char buf[24];
        snprintf(buf, 24, "%.8X vs. %.8X", stored_crc, crc_payload.get());
        throw string("t_luigi_block:  Checksum mismatch: ") + buf;
    }
#endif
}
...
void t_luigi_mem_map_table::decode( const t_luigi_block &block )
{
    ...
    {
        char buf[11];
        snprintf( buf, 11, "%u",  unsigned(enc_data.size()) );
    ...
    for (int i = 0; i < 256; ++i, ++enc_data_i)
    {
        ...
        {
            char buf[4];
            snprintf( buf, 4, "%.2X", unsigned(perm_byte) );
...
void t_luigi_mem_map_table::encode( t_luigi_block &block ) const
{
    ...
    for (int i = 0; i < 256; ++i, ++enc_data_i)
    {
        ...
        {
            char buf[60];
            snprintf( buf, 60, "para %.2X => %.4X", i, w );
    ...
    for (int i = 0; i < 16; ++i)
        for (int j = 0; j < 16; ++j, ++enc_data_i)
        {
            ...
            {
                char buf[60];
                snprintf( buf, 60, "chap %.X page %.X => %.4X", i, j, m );
                throw string("t_luigi_mem_map_table: "
...
void t_luigi::deserialize( t_locutus &locutus, const t_byte_vec &data )
{
    ...
    {
        char buf[10];
        snprintf(buf, 10, "%.2X vs. %.2X", crc_header_stored,
    ...
    while (data_i != data_e)
    {
        ...
        switch ( block.get_type() )
        {
            ...
            {
                char buf[4];
                snprintf( buf, 4, "%.2X", block.get_type() );
